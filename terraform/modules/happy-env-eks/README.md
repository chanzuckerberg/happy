...
| <a name="input_rds_dbs"></a> [rds\_dbs](#input\_rds\_dbs) | Map of DB's to create for your happy applications. If an engine\_version is not provided, the default\_db\_engine\_version is used | <pre>map(object({<br>    name           = string,<br>    username       = string,<br>    instance_class = string,<br>    engine_version = string,<br>    rds_cluster_parameters = tuple(object(<br>      {<br>        name         = string<br>        value        = string<br>        apply_method = string<br>      }<br>    )),<br>  }))</pre> | `{}` | no |
