...
| <a name="input_volume_mounts"></a> [volume\_mounts](#input\_volume\_mounts) | Map of volume mounts to be mounted in the service | <pre>map(object({<br>    mount_path : string,<br>    read_only : optional(bool, false)<br>  }))</pre> | `{}` | no |
| <a name="input_volumes"></a> [volumes](#input\_volumes) | Map of volumes to be mounted in the service | <pre>map(object({<br>    type : optional(string, "EMPTY_DIR"), // Supported values: EMPTY_DIR, CONFIGMAP, SECRET<br>    ref : optional(string, "")<br>  }))</pre> | `{}` | no |
